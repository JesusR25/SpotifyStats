...
import { Download, Sparkles, Calendar, ChevronLeft, ChevronRight, ChevronUp, ChevronDown } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="flex justify-center space-x-4">
            <Button onClick={downloadAll} variant="primary" size="lg">
        ...
        <div className="relative flex items-center justify-center min-h-[600px]">
          ...
          <button
            onClick={prevCard}
            className={`absolute z-10 p-3 rounded-full bg-spotify-lightGray/80 hover:bg-spotify-lightGray text-white transition-all ${
              isVertical ? 'top-4 left-1/2 -translate-x-1/2' : 'left-4'
            }`}
          ...
          <div className={`relative w-full max-w-[1080px] ${isVertical ? 'h-[600px]' : 'h-[1080px]'} overflow-hidden`}>
            <AnimatePresence mode="wait">
              {/* Card 1: Portada */}
              {currentCardIndex === 0 && (
                <motion.div
                  key="card-0"
                  initial={{ opacity: 0, x: isVertical ? 0 : 300, y: isVertical ? 300 : 0 }}
                  animate={{ opacity: 1, x: 0, y: 0 }}
                  exit={{ opacity: 0, x: isVertical ? 0 : -300, y: isVertical ? -300 : 0 }}
          ...
          <button
            onClick={nextCard}
            className={`absolute z-10 p-3 rounded-full bg-spotify-lightGray/80 hover:bg-spotify-lightGray text-white transition-all ${
              isVertical ? 'bottom-4 left-1/2 -translate-x-1/2' : 'right-4'
            }`}
        ...
        <div className={`flex justify-center mt-8 space-x-2 ${isVertical ? 'flex-row' : 'flex-row'}`}>
